...
--	local pan	= sesys_pan(p.pan or 0)				-- pan	三桁 / 左-100 中0 右100
	...
	if not csv.extra_bgm[name] then
		message("通知", name, "は登録されていないファイルです")
		return
	else
...
--		message("通知", name, "は既に再生されています")
	...
	-- fade処理(noise対策)
	local tm = time or 0
	local mx = init.game_bgmfadein or 0
	if tm < mx then tm = mx end

	local cf = s.file
	if cf then	e:tag{"sxfade", file=(':bgm/'..file..game.soundext), time=(tm), loop=(loop), gain=(gain), buffer=(sesys_getbuffer("bgm"))}
	else		e:tag{"splay",  file=(':bgm/'..file..game.soundext), time=(tm), loop=(loop), gain=(gain), buffer=(sesys_getbuffer("bgm"))} end
	epan()
