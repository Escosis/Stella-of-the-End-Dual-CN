...
function mw_text(v)
	...
	-- text処理
		...
		if i == 1 then
			...
			estag{"mw_nameflip", p}		-- 名前表示 / flip
			estag{"flip"}				-- 
			estag{"mw_advtext", p}		-- 本文表示
			estag{"image_loop"}			-- 画像処理
			estag()
		end
	end
end
...
function mw_nameflip(p)
	...
	local im = init.mw_namemode == "img"
	local id = getMWID("name")
	if n and not nv and (id or im) then
		flip()
	end
end
----------------------------------------
...
function message_control(tbl, flag)
	...
	tag{"chgmsg", id=(id)}
	mw_areadfastcheck()
	estag{"mw_textloop", { text=(tbl), lang=(ln) }}
	if flag then
		estag{"flip"}
		estag{"eqwait"}
	end
	...
	if init.game_sublangview == "on" and sb and tbl[sb] then
		r = get_artemis("get_message_layer_height")
	end
	estag{"/chgmsg"}
...
--			or flag == "line" and	init.line_aread_color	-- line
		...
		----------------------------------------
		-- テキスト描画
		local md = p.mode ~= "quick"
		local r  = {}
		if md then estag("init") end
		for i, t in ipairs(v) do
			if type(t) == "table" then
				...
				elseif tags[s] then 
					...
					table.insert(r, {"tags", t})
				end
			else
				table.insert(r, {"tags", {"print", data=(t)}})
			end
		end
		if cx then table.insert(r, {"tags", {"/font"}}) end
...
----------------------------------------
-- 名前ベース取得
function getNameChar(p, mode)
	local r  = nil

	-- 立ち絵
	if type(mode) == "string" then
		local ln = init.game_language or "ja"		-- デフォ言語
		local z  = p and p[ln] and p[ln][1] and p[ln][1].name
		if z then
			local nm = "game_checkname"..mode		-- 仮名確認
			r = init[nm] == "on" and z[2] or z[1]	-- 設定によって返す名前を変える
		end

	-- 現在の名前を返す
	else
		local ln = get_language()					-- 現在の言語
		local z  = p and p[ln] and p[ln][1] and p[ln][1].name
		if z then
			r = z[2] or z[1]
		end
...
----------------------------------------
function txkey_click()	tag{"@"} end
function txkey_exit()	flg.txclick = nil end
----------------------------------------
-- 外字
function gaiji(p)
	local tx = p.text
...
-- ruby
function txruby(p)
	local r = conf.ruby or 1
	if r == 1 then
		local tx = p.text
		if tx then tag{"ruby", text=(tx)} else tag{"/ruby"} end
	end
end
----------------------------------------
-- font
function exfont(p)
	...
	-- 閉じる
	if not sz and not co and not st and not al then
		tag{"/font"}
		scr.fsize = getFontSize()

	-- align
	elseif al then
		local z = {"font", align=(al)}
		tag(z)
...
-- text code
function txcode(p)
	local tx = string.format("%c", p.data)
	tag{"print", data=(tx)}
end
----------------------------------------
--
