...
function tn2(n)
	local r = n
	if type(n) == "string" and n:find("^[0-9%.]+$") then r = tonumber(n) end
	return r
end
----------------------------------------
...
----------------------------------------
-- ■ get_artemis
function get_artemis(md, p)
	local param_string = { os=1, file_crc=1 }	-- stringで返す名前を記述
	local pn = "t.param"

	-- varタグ
	local z  = {"var", name=(pn), system=(md)}
	local sw = {
		file_crc		= function() z.file = p end,
		get_layer_info	= function() z.id = p end,
	}
	if sw[md] then sw[md]() end
	tag(z)					-- 実行
	local r = e:var(pn)		-- 戻り値
	...
	local sw = {
		...
		-- date
		date = function()
			local y = tn(e:var(pn..".year"))
			local m = tn(e:var(pn..".month"))
			local d = tn(e:var(pn..".day"))
			local h = tn(e:var(pn..".hour"))
			local t = tn(e:var(pn..".minute"))
			local s = tn(e:var(pn..".second"))
			r = { y, m, d, h, t, s }
		end,

		-- get_layer_info
		get_layer_info = function()
			local x = tn(e:var(pn..".left"))
			local y = tn(e:var(pn..".top"))
			local w = tn(e:var(pn..".width"))
			local h = tn(e:var(pn..".height"))
			local a = tn(e:var(pn..".alpha"))
			r = { x=(x), y=(y), w=(w), h=(h), a=(a) }
		end,
	}
	if sw[md] then
		sw[md]()
	elseif not param_string[md] then
		r = tn(r)
	end
	tag{"var", name=(pn), system="delete"}
	return r
end
----------------------------------------
-- ■ screen min toggle / 最小化ボタン
function screen_min()
...
function screen_change()
	if get_artemis("fullscreen") == 0 then
		fullscreen_on()
	else
		fullscreen_off()
	end
...
function fullscreen_on()
	...
	if md == "windows" then
		if cw == 1 and get_artemis("fullscreen") == 0 then tag{"exec", command="fullscreen"} end
	elseif md == "wasm" then
...
function fullscreen_off()
	...
	if steam and steam.is_running_on_deck() then
		-- steamdeckでは実行しない
	elseif md == "windows" then
		if cw == 0 and get_artemis("fullscreen") == 1 then tag{"exec", command="fullscreen"} end
...
----------------------------------------
-- ■数値を値段に変換
function NumToPrice(num, delim)
	local no = ts(num or 0)
	local de = delim or ","
	local ct = #no
		if ct > 6 then no = no:gsub("(%d+)(%d%d%d)(%d%d%d)", "%1"..de.."%2"..de.."%3")
	elseif ct > 3 then no = no:gsub("(%d+)(%d%d%d)", "%1"..de.."%2") end
	return no
end
----------------------------------------
...
----------------------------------------
-- ■ NumToGrph関数 / 数値を桁ごとに分解(４桁)
function NumToGrph4(num)
	local dh = 0 if num >= 1000 then dh = math.floor(num / 1000) num = num % 1000 end
	local dl = 0 if num >= 100  then dl = math.floor(num / 100 ) num = num % 100  end
	local ah = math.floor(num / 10)
	local al = num % 10
	return { dh, dl, ah, al }
end
...
--		r.message = ""
		if r.varname then
			message("警告", "SwitchではCancelボタンが使用できません")
			tag{"val", name=(r.varname), data="1"}
			r.varname = nil
		end
		if not r.textfield then
			message("警告", "Switchでは文字入力のみ使用可能です")
			r.textfield = "t.tmp"
		end

	-- wasm処理
	elseif game.wasm then
		if not r.varname then
			message("警告", "wasmではOKボタンのみにはできません")
			r.varname = "t.tmp"
		end
	end
	tag(r)
...
----------------------------------------
-- ■ ClearMemory
function ClearMemory(mode)
	local v  = init.system
	local ct = math.ceil(collectgarbage("count") / 1024)	-- 容量を取得
	local mx = getWindowsOSbit() and v.lua_cache_x64 or lua_cache or 10

	-- 容量超過 or UIから抜けたとき
	if ct >= mx or mode == "clear" then
		collectgarbage("step", 0.01)
