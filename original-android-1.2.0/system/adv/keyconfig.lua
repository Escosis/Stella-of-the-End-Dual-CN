...
function getKeyString(p)
	...
	local st = ""
	if nm == "ALL" then tb = csv.advkey.def end
	if tb then
		for i, v in pairs(tb) do
			if st == "" then st = ""..i else st = st..","..i end
		end
	else
		error_message(nm.."がキーリストに存在しませんでした")
	end
	return st
end
----------------------------------------
...
function setonpush_ui(flag)
	-- ui
	if flg.ui or flg.dlg or flg.dlg2 then
		delonpush_ui()
...
--		e:tag{"setonpush", key="117", handler="calllua", ["function"]="setonpush_calllua"}
	...
	tag{"keyconfig", role="0",  keys=""}		-- click停止
end
----------------------------------------
-- select
...
function setonpush_calllua(e, param)
	...
	----------------------------------------
	-- 右フリック処理
	if key == 152 and init.game_flickback == "on" and (flg.ui or flg.dlg or flg.dlg2) then
	...
	----------------------------------------
	-- debug key
	if debug_flag and flg.debugfgimage then
		debugFGKeyin(param)
		return
	elseif debug_flag and not flg.alt then
...
--[[
	...
	----------------------------------------
	-- dialog
	elseif flg.dlg or flg.dlg2 then
		...
		-- 呼び出し
		if flg.dlg2 then
			local tbl = { CLICK="yesno_exclick", EXIT="yesno_exclick", F2="yesno_checkbox", LT="btn_left", RT="btn_right", UP="btn_up", DW="btn_down" }
			if tbl[name] then callfunc(tbl[name], key, name, btn.cursor) end
		elseif flg.dlg.sbox then
			local tbl = { CLICK="config_selectbox_click", EXIT="config_selectbox_close", UP="config_selectbox_up", DW="config_selectbox_dw" }
			if tbl[name] then callfunc(tbl[name], key, name, btn.cursor) end
		else
	...
	elseif flg.ui then
		...
		-- CLICK読み替え
		local kx = n and btn[n] and btn[n].key or {}
			if key == 13 and kx.ENT then name = "ENT"
		elseif key == 1  and kx.LCL then name = "LCL" end
...
function exkeyin(p)
	...
	e:enqueueTag{"keyconfig", role="0", keys=(getexclick())}		-- dummy click
...
function exkeyin_exit(e, p)
	setonpush_init()
--	e:enqueueTag{"keyconfig", role="0", keys=""}		-- dummy click
