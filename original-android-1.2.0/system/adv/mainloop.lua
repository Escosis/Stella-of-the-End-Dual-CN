...
function scriptMainAdd()
	...
	if c <= m then
		...
		flg.exskiptween = nil	-- exskip実行時のtween抑制フラグ
...
function clickAutomode()
	...
	local cp = t.clickpass
	if cp then
		if cp < 0 then
			local z = scr.sesys.buff or {}
			for id, v in pairs(z) do
				if v.mode == "voice" then
					eqwait{ se=(id), input="1"}
				end
			end
		else
			eqwait(cp)
		end
		eqwait{ scenario="1", input="1"}
		eqtag{"jump", file="system/script.asb", label="click3"}

	-- fastauto
	elseif flg.fastauto then
		local tm = getFastautoTime(1000)
		eqtag{"wait", time=(tm), input="0"}
--		eqtag{"wait", scenario="1", input="0"}						-- テキスト待機するとめっちゃ遅くなる
		eqtag{"jump", file="system/script.asb", label="click3"}		-- @も同様
	...
	elseif flg.skipmode then
		...
		if s then
			-- テキスト待機を有効にする
			if init.skip_textwait == "on" then
				eqtag{"wait", input="0", scenario="1" }
			end

			-- テキスト待機時間
			local st = init.skip_textmagni or 2
			local tm = s * st
			if tm > 0 then
				eqtag{"wait", input="0", time=(tm)}
			end
		end
	...
	elseif not getSkip() then
		...
		if _G[s] then _G[s]() else glyph_set() flip() end
	end
end
----------------------------------------
-- クリック直後の処理
...
function checkAread()
	...
	if ar ~= scr.areadflag then
		-- 既読マーク
		local a = ar and conf.aread_icon == 1 and 255 or 0
...
function tag_scene(p)
	...
	["end"] = function()
		...
		if getExtra() then
			...
			if emote then emote.bgreset() end		-- E-mote reset
