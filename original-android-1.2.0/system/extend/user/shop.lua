...
function netshop_ui(z)
	...
	if not flg.purchase then
		...
		local d  = getLangHelp("dlgmes")
		local tx = d[t[2]] or "none"
		local tl = d[t[3]] or "none"
		local yn = flg.price or t[4] or 9999999
		if tx == "none" then tx = "" end
		if tl == "none" then tl = "" end

		purchase_reseterror()		-- error reset
