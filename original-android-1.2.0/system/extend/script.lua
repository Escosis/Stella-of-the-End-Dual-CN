...
function tags.txruby(e, p)	txruby(p)		return 1 end	-- 
...
function tags.permission(e, p)
	...
	menu = function()
		if bn == 1 then
			message("通知", "menuを禁止しました")
			menuoff()
		else
			message("通知", "menuを許可しました")
			menuon()
		end
	end,
	...
	tweet = function()
		if bn == 1 then
			message("通知", "tweetを禁止しました")
			scr.scenearea = true
		else
			message("通知", "tweetを許可しました")
			scr.scenearea = nil
		end
	end,
	...
	shoot = function()
		if cs then
			if bn == 1 then
--				message("通知", "撮影を禁止しました")
				cs.watermark("off")
			else
--				message("通知", "撮影を許可しました")
				cs.watermark("on")
			end
		end
	end,
	...
	emote = function()
		if bn == 1 then
			message("通知", "E-moteの動作を禁止しました")
			scr.emotemotion = true
		else
			message("通知", "E-moteの動作を許可しました")
			scr.emotemotion = nil
		end
	end,
	}
	if sw[md] then
		...
		message("警告", md, "は存在しないpermissionです")
	end
	return 1
end
----------------------------------------
