...
function get_eval(text)
	...
	elseif head == 's' then
		...
		-- steam
		elseif name == "steam" and init.steam == "on" then ret = 1
		elseif name == "deck" and steam and steam.is_running_on_deck() then ret = 1
--		elseif name == "dmmpl" and init.dmmpl_exe then ret = 1
--		elseif name == "softdc" and init.softdc_exe then ret = 1
...
function exskip_stop(nm)
	if flg.exskip then
		stack_eval()							-- 変数に更新があればスタックしておく
		flg.exskipip = tcopy(scr.ip)			-- 現在位置を保存しておく
		if nm then flg.exskipcache = true end	-- 未読停止
