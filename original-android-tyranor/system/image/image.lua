...
function readImage(id, p, tone)
	...
	if id and p.hold ~= 1 then lydel2(id) end
	...
	elseif mvfl then
		...
		local lm = p.layermode
		tag{"video" , id=(idx), file=(px), loop=(lp)}
		tag{"lyprop", id=(addImageID(id, 'move')), anchorx=(ax), anchory=(ay)}
		tag{"lyprop", id=(addImageID(id, 'act' )), anchorx=(ax), anchory=(ay)}
		if lm then tag{"lyprop", id=(idx), layermode=(lm)} end
		if id then tag{"lyprop", id=(id), visible=(anef), layermode=(p.mvlm)} end

		-- sync
		if sy == 1 and anef == 1 and not getSkip() then eqtag{"wait", video=(idx), input=(i)} end

	--------------------------------
	-- ipt
	elseif isFile(px..'.ipt') then
		e:include(px..'.ipt')
...
--			tag{"lyprop", id=(idx), left=(v.ax), top=(v.ay)}
		...
		--------------------------------
		-- ev目パチアニメーション
		anime_eveye = function(p)
			local v  = ipt.base
			local no = p.no or 1
			local t  = ipt[no]
			lyc2{ id=(idx..'.0'), file=(path..v[1]) }
			...
			if t then			
				...
				for i=1, mx do
					local ct = (i-1) * w
					local al = i ~= 1 and 0
					lyc2{ id=(idx..'.'..i), file=(path..t[1]), x=(t.x), y=(t.y), clip=(ct..wh), alpha=(al) }
				end
				tag{"lyprop", id=(idx), intermediate_render="1"}

				-- 登録
				if mx > 1 and init.game_evanime == "on" then
					set_eyesanime("evani", idx, mx, p)
				end
			else
				message("警告", "no:", no, "は登録されていない表情です")
			end
		end,
...
function image_postween(id, p, fade, flag)
	...
	local loop = p.loop
	local yoyo = p.yoyo
	local f1   = not (flag or getSkip() or flg.blj or flg.exskiptween)		-- quickjump時は停止
...
----------------------------------------
-- 目パチ登録
function set_eyesanime(md, id, mx, p, com)
	local no = md == "evani" and (p.id or 0) + 1 or p.id or 1
	local cm = com or "ani"
	if not scr[md] then scr[md] = {} end

	-- anime
	local t = {}
	for i=1, mx do table.insert(t, i) end
	if cm == "ani" then for i=mx-1, 1, -1 do table.insert(t, i) end end		-- ani(往復)

	-- 登録
	local tm = e:now() + get_eyesanimetime()
	scr[md][no] = {
		id = (id),		-- image id
		ct = 1,			-- counter
		tm = (tm),		-- 次の時間(間隔)
...
function del_eyesanime(md, id)
	...
	if p then
		...
		if id then
			...
			-- 全停止確認
			if tmaxn(scr[md]) == 0 then scr[md] = nil end

		-- 全停止
		else
			for no, v in pairs(p) do stop_eyesanime(v) end
			scr[md] = nil
		end
	end
end
----------------------------------------
-- 目パチ / vsync
function vsync_eyesanime(md)
	local p = scr[md]
	if p then
		local nw = e:now()
		local wa = init.game_eyesanime_wait or 30		-- アニメーション時間

		-- loop
		for no, v in pairs(p) do
			if v.tm < nw then
				local id = v.id.."."
				local ct = v.ct
				local cx = ct + 1
				...
				-- 最後まで行った
				if cx >= mx then
					tm = get_eyesanimetime(md, no)
					cx = 1
				end
				tag{"lytween", id=(id..t[ct]), param="alpha", time="0", from="1", to="0"}
				tag{"lytween", id=(id..t[cx]), param="alpha", time="0", from="254", to="255"}
				scr[md][no].ct = cx
				scr[md][no].tm = tm + e:now()
			end
		end
	end
end
----------------------------------------
-- 目パチ停止
function stop_eyesanime(v)
	local ct = v.ct
	if ct > 1 then
		local t  = v.t
		local ix = v.id.."."
		tag{"lytween", id=(ix..t[ct]), param="alpha", time="0", from="1", to="0"}
		tag{"lytween", id=(ix..t[1 ]), param="alpha", time="0", from="254", to="255"}
	end
end
----------------------------------------
-- 目パチ時間を取得
function get_eyesanimetime(md, no)
	local tm = init.game_eyesanime_time or 3000		-- アニメーション間隔
	local wa = init.game_eyesanime_wait or 30		-- アニメーション時間
	local fq = init.game_eyesanime_freq or 20		-- 二度まばたき頻度
	if md then
		-- 低確率で連続する
		if not scr[md][no].eye and (e:random() % 100) < fq then
			scr[md][no].eye = true
			tm = wa
		else
			scr[md][no].eye = nil
			tm = math.floor(tm / 2) + e:random() % tm
		end
	end
	return tm
end
