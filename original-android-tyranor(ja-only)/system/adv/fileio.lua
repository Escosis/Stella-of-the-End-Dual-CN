...
function fload(file, flag)
	...
	if game.trueos == "windows" then
		...
		-- 読み込み
		local p = e:file(path..file)
		if p then r = pluto.unpersist({}, p) end
	else
		local p = e:var("g.debfsave."..file)
...
function fsave(file, tbl, flag)
	...
	if game.trueos == "windows" then
		...
		tag{"var", name=("g.debfsave."..file), data=(z)}
	end
end
----------------------------------------
-- ■ fload_pluto関数
