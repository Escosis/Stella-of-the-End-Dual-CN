...
-- onEnterFrame制御
----------------------------------------
local touchout = { 1, 139, 140, 143, 151, 152, 153, 154 }
----------------------------------------
...
-- vsync / 常に走らせるもの
function vsync()
	flg.alt = game.pc and e:isDown(18)		-- alt保存

	-- touch処理
	if e:isDownEdge(1) then
		flg.m	  = e:getMousePoint()		-- クリック位置を記録
		flg.touch = e:getTouchCount()		-- touch/click判定
		flg.touchover = true				-- touch判定フラグ
	elseif flg.touchover then
		for i, v in ipairs(touchout) do
			if e:isUpEdge(v) then
				flg.touchover = nil
				flg.sliderdate = e:now() + 50	-- touch判定を少し残す
				break
			end
		end
	elseif flg.sliderdate and flg.sliderdate < e:now() then
		flg.sliderdate = nil				-- touch判定開放
		flg.sliderdrag = nil				-- slider開放
	end
...
--						e:overrideKey{ key=(2), status=32 }
	...
	----------------------------------------
	-- skip停止漏れ対策
	if not flg.autoskipvsync and (flg.skipmode or flg.automode or flg.fastauto) then
	...
	----------------------------------------
	-- scetweendel
	if flg.scetweendel then
		local s = flg.waitparam or {}
		if s.textTween then
	...
	adv = function()
		...
		----------------------------------------
		-- 目パチ
		if init.game_evanime == "on" then vsync_eyesanime("evani") end
		if init.game_fganime == "on" then vsync_eyesanime("fgani") end
	...
	ui = function()
		...
		----------------------------------------
		-- bgm ctrl
		elseif bgmctrl and flg.bgmctrl then
			bgmctrl.vsync()
--[[
...
function vsync_function()
	----------------------------------------
	-- mouse
	local nm = flg.mousefunc
	if nm and _G[nm] then _G[nm]() end

	----------------------------------------
	-- tap effect
	if tapeffect then vsync_tapeffect() end
