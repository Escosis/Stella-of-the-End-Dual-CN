...
--		message("通知", i, c, f, text, v.cond)
	...
	local n = "selectbg"..c
	local b = csv.mw[n]
	if b then lyc2{ id=(id..'.0'), file=(get_uipath(b.file)), clip=(b.clip), x=(b.x), y=(b.y)} end
...
function select_clicknext()
	...
	local as = s.autoselect
	if not as then
		restart_autoskip()		-- auto/skip再開
		set_message_speed()		-- mspeed復帰
		allkeyon()				-- 入力許可
	else
		local nm = flg.automode and "auto" or flg.skipmode and "skip"
		if nm then autoskip_startimg(nm) end
	end
...
function goNextSelect()
	...
	ClearMemory("clear")			-- ガベージコレクション実行
