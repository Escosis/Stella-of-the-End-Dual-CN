	...
	if p then
		...
		if s then
			for i, v in pairs(s) do
				local ch = v.ch
				local fl = v.file
				local t  = ch and csv.voice[v.ch]

				-- 各言語ごとにsliを読み分ける
				local ln = get_language("voice")	-- 音声言語を取得
				local z  = init.langvo or {}
				if init.game_langslipath == "on" and ln and z[ln] and z[ln].voice then
					local ad = z[ln].voice or ""
					fl = ad..fl
				end

				-- sli読み込み
				local r  = opensli(t.path..fl)
				if r then
					scr.sli.time = r
					scr.sli.file = v.file
					scr.sli.id   = sesys_getvoiceid(ch)
					break
				end
			end
...
function delay_end()
	...
	-- exclose
	if flg.delayexclose then
		flg.delayexclose = nil
		window_exclose()
	end
end
----------------------------------------
