...
function restart_autoskip()
	...
	if scr.adv.memory then
		local s = flg.selectskip or {}

		-- skipmodeの再設定
		if conf.skip == 1 and scr.adv.memory.skip or s.skip then
...
--			menu_hide()
		...
		-- automodeの再設定
		elseif conf.auto == 1 and scr.adv.memory.auto or s.auto then
			automode_start()
--			menu_hide()
...
function skipmode_start()
	...
	if flg.skipstart or scr.areadflag or sk == 1 then
		...
		-- fastauto
		if conf.fl_fastauto == 1 then
			flg.fastauto = true
			set_message_speed()
			tag{"automode", allow="1", stopbyclick="1", stopbystop="1", syncse=""}
			tag{"exec", command="automode", mode="1"}

		-- skip
		else
			-- 動作設定(固定
--			tag{"skip", allow="1", unread=(sk)}
			tag{"skip", allow="1", unread=(1)}
