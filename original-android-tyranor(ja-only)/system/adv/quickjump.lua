...
--			reset_voice()
	...
	for i, nm in ipairs(ex.tag_table) do
		...
		sw.fg = function(p)
			...
			for id, t in pairs(p) do
				...
				if a then
					...
					-- mode
					local ch = a.ch
					if ch then scr.img.mwf[ch] = m end
		...
		-- flash
		sw.flash = function(p)
			local a = checkscript('flash', p)
			if a then flash(a) end
		end
		...
		----------------------------------------
		-- permission
		sw.permission = function(p)
			for id, t in pairs(p) do
				local a = checkscript('permission', t)
				if a then tags.permission(e, a) end
			end
		end
...
--	setMWImage(mwnm)
	...
	if chk then
		...
		elseif flag == "exskip" then
			local s = flg.exskipip or {}	-- 直前の位置
			count = s.count or 1
			flag = "ui"
			flg.exskipjump = true			-- exskipjumpフラグを立てておく
			flg.exskiptween = true			-- exskip実行時のtween抑制フラグ
		...
		-- UIから呼び出し
		elseif flag == "ui" or flag == "bsel" then
			scr.uifunc = nil
			flg.ui = nil
...
function quickjumpnovel(p)
	...
	if no and no >= 1 then
		...
		for i=no, 1, -1 do
			...
			if not tx.hide then
				-- name
				local nm = getNameChar(tx)
				if nm then message_name(nm) end

				-- text
				local ln = get_language()
				mw_textloop{ text=(tx), lang=(ln), mode="quick" }
...
--[[
	...
	sw.fg = function(p)
		...
		elseif md == -1 then
			...
			-- face削除確認
			local s  = scr.face or {}
			local mf = init.mwfacefgid or 200		-- mwfaceはid決め打ち
			if s[ch] and (md == 2 or emote and md == 3) then stackBLJ('fg', nil, mf) end
	...
	sw.flash = function(p)
		local lp = p.loop
		local md = p.mode
		if lp == -1 then		 stackBLJ('flash', getBlock(dl))
		elseif md == "stop" then stackBLJ('flash', nil) end
	end
...
--	dump(p)
	...
	sw.permission = function(p)
		...
		local bn = p.ban or 0
		if bn == 0 then
			stackBLJ('permission', nil, md)
		else
			stackBLJ('permission', getBlock(dl), md)
		end
	end
