...
--		lydel2(idx)
		...
		tag{"lyprop", id=(idx), intermediate_render="2"}
		...
		for nm, id in pairs(init.fgid) do
			...
			elseif not ex then
				...
				if init.game_fganime == "on" and a then
					...
					set_eyesanime("fgani", ix, #a, p, f.com)
				else
...
--		else				 setTimezone(idx, ch) end		
		...
		-- 境界線対策
		if init.game_fgsynthesis == "on" then tag{"lyprop", id=(idx), intermediate_render="1"} end

		-- lymask
		local ms = p.lymask
		if ms then tag{"lyprop", id=(id), intermediate_render="2", intermediate_render_mask=(":mask/"..ms)} end
	end
...
function fgf(p)
	...
	if t then
		...
		----------------------------------------
		-- mask
		tag{"lyprop", id=(idx), intermediate_render="2", intermediate_render_mask=(':mask/'..p.frame)}
		...
		if ch then scr.img.fgf[ch] = { id=(id), idx=(idx), fx=(mulpos(t.fx)), zone=(zn), bg=(file) } end
...
--			tag{"lyprop", id=(idm), anchorx=(mulpos(t.fx)), anchory=(mulpos(t.fy))}
		...
		if ch then
			local z = scr.img or {}
			local n = s[ch] and s[ch].fgid or 1
			del_eyesanime("fgani", n)
...
-- 音声のあるキャラのみmwに出す
function faceview(p)
	local ch = scr.faceflag or getNameChar(p, "fg")
...
function getMWFaceFile(p, flag)
	local tbl  = init.fgid
	local file = (patch_checkfg() and p.ex05 or p.file):gsub("_[bnz][co0-9]", "_no")
	local head = file:sub(1, 7)
--	local head = file:gsub("_no", "_fa"):sub(1, 7) if p.head == "mob" then head = "mob" end
	local path = p.path:gsub(":fg", ":fa"):gsub("/[bnz][co0-9]/", "/fa/")
...
function setMWFaceFile(p, nm, id)
	...
	local md   = init.system.fgmode == "csv"
--	local idb  = id..".0"
	local idb = addImageID(id, 'pos')
	local path = p.path
	local head = md and (p.file.file:sub(1, 7)):gsub("_[bnz][co0-9]", "_fa")
	local z    = md and fgpos[head]
	local ext  = game.fgext
	local mw   = csv.mw[nm]
	...
	if nm then
		...
		if nm == "face" then
			...
			if f == "none" then
				...
				tag{"lyprop", id=(id), left=(mw.x), top=(mw.y), intermediate_render="2", intermediate_render_mask=(m)}
			end

			-- faceにも色調を使用する / シェーダー反映
			if init.game_mwtone == "on" then
				local t = scr.tone
				if t then
					shader_colortone(id, t.p)
				else
					shader_colortone(id, p)
				end
			end
		else
			local r = c and 1
		...
		tag{"lyprop", id=(idb), intermediate_render="2", anchorx="0", anchory="0", xscale=(z), yscale=(z)}	-- ここの中間合成は必須
	end
end
----------------------------------------
-- ボイスを取得して立ち絵の表情を返す
function getBlogFace()
	...
	local t  = ast[b].text
	local nm = getNameChar(t, "blog")		-- 話者名
	if nm and v[nm] then
