...
function reset_bg(flag)
	...
	del_eyesanime("evani")	-- ev anime停止 / eyes
	del_eyesanime("fgani")	-- fg anime停止 / eyes
	scr.img = { bg={}, fg={}, fgf={} }
...
--	loading_off()
	...
	tag{"lydel", id="startmask"}
	tag{"lydel", id="zchapter"}

	-- 立ち絵削除
	if not hl then
	elseif no == 1 and p.face ~= 1 then
		-- bg/ev表示時はbg通知を消す
		notification_clear("bg")		-- 背景名
--		fgdelall()						-- 立ち絵消去
--		timezone(p)						-- 立ち絵フィルタ
--		reset_bg(p.sync == 0)			-- resetしておく
	elseif no == 1 then	del_eyesanime("evani")	-- ev animeだけ停止
	else				del_eyesanime("evani", no) end

	-- siglus trans
	local ru = p.rule or ""
	if no == 1 and ru:find("sig_") then
		siglus_trans(id, p)
	end

	-- cg check
--	if debug_flag and no > 1 then
--		local z  = scr.img.bg[no] or {}
--		local fl = z.file
--		if fl and fl ~= file then
--			message("警告", "cgタグのidが重複しています", file, fl)
--		end
--	end
...
--		if name == "bg" then bg_f(p) end
	...
	-- tween
	if hl then image_postween(id, p) end
