...
function favopage_init()
	...
	-- mask / nodata
	local face = get_uipath("save/maskfavo", true) or get_uipath("save/maskface", true)
	local none = get_uipath("save/none"..scr.savecom, true) or get_uipath("save/none", true)
	local mask = get_uipath("save/mask"..scr.savecom, true) or get_uipath("save/mask", true)

	-- icon
	ipt = nil
	local thbg = nil
	local base = get_uipath().."save/char.ipt"
	if isFile(base) then
		e:include(base..".ipt")
		base = get_uipath("save/char")
		thbg = get_uipath("save/thbg", true)
	end
...
--		local vv = getBtnInfo('obj'..string.format("%02d", i))
		...
		if t then
			...
			local lc = true
			local fa = t.face
			local nz = tx.name[ln] or tx.name[1]

			-- none
			if fa and fa[1] == "none" then
				local s = ipt and (ipt[fa.text] or ipt[fa.name])
				if s then lyc2{ id=(thid..".1"), file=(base), x=(ss.x), y=(ss.y), clip=(s), mask=(mask)} end

			-- st
			elseif fa and fa.path and (not ipt or ipt.icon and not ipt.icon[nz]) then
				if thbg then lyc2{ id=(thid..".0"), file=(thbg), x=(ss.x), y=(ss.y)} end
				setMWFaceFile(fa, "favoface", thid..".1")
				if face then tag{"lyprop", id=(thid), intermediate_render="2", intermediate_render_mask=(face)} end

			-- icon
			elseif ipt then
				local cl = nz and ipt[nz] or ipt.etc
				lyc2{ id=(thid..".1"), file=(base), x=(ss.x), y=(ss.y), clip=(cl)}
				if face then tag{"lyprop", id=(thid), intermediate_render="2", intermediate_render_mask=(face)} end
			end
...
----------------------------------------
-- セーブクリック
function favo_click(e, p)	favo_clickex(btn.cursor) end
function favo_clickui(e, p)	favo_clickex(p.bt) end
function favo_clickex(bt)
	if bt then
