...
--		e:tag{"lyprop", id="500.z.sl", visible="0"}
		tag{"lyprop", id=(getBtnID("slider")), visible="0"}
		setBtnStat('bt_up', 'c')
		setBtnStat('bt_dw', 'c')
...
function blog_cache()
	...
	local p  = log.stack
	local m  = #p
	local ln = get_language()		-- 多言語 / メイン
	local sb = get_language("sub")	-- 多言語 / サブ
...
function view_backlog()
	...
	for i=1, pg do
		...
		if t then
			...
			if t.sub then
				local y = get_artemis("get_message_layer_height")
				e:tag{"/chgmsg"}
			...
			if v and v.file and not t.lineface then
				...
				tag{"lyprop", id=(id..".7"), left=(c.x), top=(c.y - b.y), intermediate_render="2", intermediate_render_mask=(mask)}
...
function backlog_mode(mode, del)
	...
	for i=1, init.backlog_page do
		...
		if del == 'init' then set_textfont("backlog_aread", (id)) end
		e:tag{"chgmsg", id=(id), layered=(mode)}
		if del then e:tag{"rp"} end
		e:tag{"/chgmsg"}
...
function log_sliderpos()
	if flg.blog.slider and checkBtnExist("slider") then
...
function log_btnover(e, p)
	...
	if bt then
		...
		-- color
		tag{"lyprop", id=("500.z.bt.tx."..p2..".3"), alpha="255"}
	end
end
----------------------------------------
function log_btnout(e, p)
	...
	if bt then
		...
		-- color
		tag{"lyprop", id=("500.z.bt.tx."..p2..".3"), alpha="0"}
	end
end
----------------------------------------
-- quickjump
