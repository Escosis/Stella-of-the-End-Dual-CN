...
function wasm.command(p)
	...
	if wasm.checkTrue() then
		local s = p:gsub("[\t\r\n ]", "")
		if s:find("artemis") then
			s = s:gsub("artemis%(\"([^%)]+)\"%)", function(a)
				local r = a
				if a:sub(0, 1) == "$" then
					r = e:var(a:sub(2))
				else
...
function wasm.init()
	...
	if wasm.check() then game.wasm = true end
end
----------------------------------------
-- カーソルチェック
function wasm.cursorcheck()
	if game.os == "wasm" then
		local pf = game.wasm_os
		if pf == "ios" or pf == "android" then
			tag{"mouse", left="-1", top="-1"}
		end
	end
end
----------------------------------------
