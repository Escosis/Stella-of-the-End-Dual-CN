...
function sv.save()
	...
	if steam and steam.is_running_on_deck() then
		th = nil
		alt = scr.altthumb or 0
	end

	-- サムネイル作成
	local savess = function(nm)
		local t = csv.mw[nm]
		if t then tag{"savess", file=(sv.getthumbname(file, nm)), width=(t.w), height=(t.h)} end
	end
	if th then
		savess("savethumb")		-- saveサムネイル
		savess("savethumb_l")	-- saveサムネイル(大)
		savess("savethumb_s")	-- saveサムネイル(mwpanel)
	end
	...
	if no < init.save_suspend then
		...
		sys.saveslot[no] = {
			...
			auto  = flg.autothumb,		-- auto thumb
			alt   = alt,				-- 代替サムネを表示する場合の番号
		}
...
function sv.deleteno(no)
	...
	if not game.truecs then
		...
		-- delete thumb
		local delthumb = function(nm)
			local px = sv.getthumbname(path, nm)
			if px then deleteFile(px) end
		end
		delthumb("savethumb")		-- saveサムネイル
		delthumb("savethumb_l")		-- saveサムネイル(大)
		delthumb("savethumb_s")		-- saveサムネイル(mwpanel)
	end
	sys.saveslot[no] = nil
...
----------------------------------------
-- サムネイル名末尾取得
function sv.getthumbname(file, nm)
	local r = nil
	local t = csv.mw[nm]
	if t then r = file..(t.file or "")..".png" end
	return r
end
----------------------------------------
-- 抜ける
