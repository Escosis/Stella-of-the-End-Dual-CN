...
--		map  = function() user_mapinit(p) end,	-- map
		...
		altthumb = function()	user_altthumb(p) end,
	}
...
----------------------------------------
-- セーブでSSが撮れないときの代替サムネ番号を指定するタグ
function user_altthumb(p)
	scr.altthumb = tn(p.no)
end
----------------------------------------
-- 通过读写conf表实现根据语言播放视频（绿茶汉化播放专属op，其余播放官方op）的同时维持与原始movie标签一致的跳过属性（第一次播放不允许跳过，之后允许）
function play_op01_in_story()
    if conf._op01_played == nil then
        conf._op01_played = false
    end

    local lang = get_language()
    local actual_file = (lang == 'cn_gt') and 'op01_cn_gt' or 'op01'
    local path = game.path.movie .. actual_file .. game.movieext

    if conf._op01_played then
        e:tag{"video", file=path, skip="2"}
    else
        e:tag{"video", file=path, skip="0"}
        conf._op01_played = true
    end
end