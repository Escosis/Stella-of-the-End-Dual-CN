...
exf.table = {
	...
	exfg = { ui="ui_fgmode" , init="fginit", view="fgpage", vo=true, flag="exfg" , reset="fgtxreset" },		-- 立ち絵鑑賞
}
----------------------------------------
...
--	bgm_play{ file=(init.title_bgm) }
	...
	if not sys.extr then sys.extr = { music=100, vol=(conf.bgm), buff=0 } end
	if bgmctrl then bgmctrl.read() end
...
--		message("通知", bt.."が選択されました", c, n)
		...
		local sw = {
			...
			bgm		= function() exf.clickbgm(bt, n) end,	-- bgm直接再生
...
----------------------------------------
-- lua loading
function exf.loading()
	local tbl = { "cg", "scene", "bgm", "movie", "etc", "exfg", "bgmctrl" }
	local px  = "system/extra/"
