...
function quake(p)
	...
	-- 
		...
		-- run
		run = function()
			tween{ id=(id), y=("0,"..h), time=(time), ease=(ease), yoyo="-1"}
		end,

		----------------------------------------
		-- user
		user = function() user_quake(id, p) end,
		}
...
function tag_flash(p)
	...
	local md = p.mode ~= "stop"
	local lp = p.loop ~= -1
	if md and lp and getSkip(true) then return end
...
function flash(p)
	...
	local sw = {
		...
		haru = function(p)
			...
			systween{ id=(id), alpha=(a2..","..al), yoyo=(lp), time=(tm), delete="1" }
			flip()
		end,

		-- siglus ev_bright_in
		siglus = function()
			local tm = p.time or 100
			local al = p.alpha or 255;
			lyc2{ id=(id), width=(game.width), height=(game.height), color=(cl), layermode="screen"}
			systween{ id=(id), alpha=al..",0", time=(tm), delete="1" }
			flip()
		end,
	}
