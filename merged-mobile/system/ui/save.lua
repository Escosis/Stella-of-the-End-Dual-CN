...
--		tag{"lyprop", id="500.up", visible="0"}		-- qsaveはページなし
	...
	-- ページ番号書き換え
--[[
	local v  = getBtnInfo('icon')
	local vx = getBtnInfo('bt_page'..string.format("%02d", nb))
	tag{"lyprop", id=(v.idx), left=(v.x + vx.x), top=(v.y + vx.y)}
]]
	if tp then
		local px = qs and "" or string.format("%02d / %02d", nb, init.save_page)
		if nb == 21 then
			ui_message('500.z.pageno', { tp, text="AUTO" })
		else
...
--		local lockid = getBtnID("lock0"..i)
		...
		if t then
			...
			local evm = t.evmask
			if as then
				local pppx = ":thumb/"..(t.auto or "auto_thumb00")
				lyc2{ id=(thid..".0"), file=(pppx), x=(ss.x), y=(ss.y), mask=(mask)}

			-- HEVマスク
			elseif hev and evm then
				local pppx = ":evmask/"..evm
				lyc2{ id=(thid..".0"), file=(pppx), x=(ss.x), y=(ss.y), mask=(mask)}

			-- 代替サムネ
			elseif t.alt then
				local f = string.format("auto_thumb%02d", t.alt)
				
				-- auto_thumb00だけ言語別に分かれてない
				if f ~= "auto_thumb00" then
					local s  = init.langadd[get_language()]
					if s then f = f..s end
				end
			...
			-- text
			local r = save_gettexttable(t)
			local ttmx = init.save_message_max
			local time = r.date
			local tttl = r.titl
			local tttx = r.text or ""
			local tttx, ttfl = mb_substr(tttx:gsub("\n", ""), 1, ttmx)
			if ttfl then tttx = tttx.."..." end
			...
			-- 当たり判定
			if win and getBtnThreshold(v, "hit") then
				tag{"lyprop", id=(id..'.0'), clickablethreshold="255"}
			end

			-- newマークを付ける
			if newt and news and (no == last or as and asls == i) then
		...
		-- edit
		local vs = (qs or flg.save.move) and 0 or 1	-- ed
		local et = (as or ed == 0) and 'e'
		local de = (as or ed == 0) and 'c'
...
--		if checkBtnExist("bt_del" ..nx) then tag{"lyprop", id=(getBtnID("bt_del" ..nx)), visible=(vs)} end
		if checkBtnExist("bt_del" ..nx) then setBtnStat("bt_del" ..nx, de) end
		if checkBtnExist("bt_edit"..nx) then
			local nn = "bt_edit"..nx
			setBtnStat(nn, et)
			if et ~= "e" and save_lockcheck(i) then
				btn_clip(nn, 'c')
			end
		end
	end
	...
	-- タイトル画面
	if getTitle() then
--		if checkBtnExist('bt_title') then setBtnStat('bt_title', 'd') end
		if checkBtnExist('bt_save')  then setBtnStat('bt_save' , 'd') end
--		tag{"lyprop", id=(getBtnID("bt_save")), visible="0"}
	end
	tag{"lyprop", id=(getBtnID("num01")), visible="0"}
	tag{"lyprop", id=(getBtnID("num02")), visible="0"}
	siglus_titlebtn("bt_title", 'd')
end
----------------------------------------
-- text整形
function save_gettexttable(t)
	local r  = {}
	...
	-- save
	r.titl = sv.changesavetitle(t.title or {})
	r.date = get_osdate("%Y/%m/%d %H:%M", t.date)
	r.text = mb_substr(tttx, 1, mx)
	return r
end
----------------------------------------
-- max page
...
----------------------------------------
-- lock over
function save_lockover(e, p)
	local bt = p.name
	if bt then
		local lv = getBtnInfo(bt)
		local no = tn(lv.p2)
		if save_lockcheck(no) then
			e:tag{"lyprop", id=(lv.idx..".0"), clip=(lv.clip_d)}
		end
	end
end
----------------------------------------
...
function saveload_reload()
	...
	local sw = {
		save  = function()
			local no = flg.savenum
			if no then
				local v  = getBtnInfo(string.format("bt_save%02d", no))
				local ss = csv.mw.savethumb			-- サムネイル位置
				local id = v.idx.."."..ss.id		-- サムネイルid
				lydel2(id)
				flip()
			end
			flg.savenum = nil
...
function save_thover(e, p)
	...
	if v and th then
		...
		if tb and isSaveFile(pg) then
			local hev  = init.game_evmask == "on"
			local evm  = tb.evmask
			local file = sv.getthumbname(fp..tb.file, "savethumb_l")
...
----------------------------------------
-- セーブクリック
function save_click(e, p)	save_clickex(btn.cursor) end
function save_clickui(e, p)	save_clickex(p.bt) end
function save_clickex(bt)
	if bt then
		...
		local sw = {
			...
			back  = function() save_clickret()  end,
			...
			pagenew = function()
				se_ok()
				local pg = sys.saveslot.page or 1
				save_pagechange(pg)
			end,
...
----------------------------------------
-- 
function save_numimage(id, v, no, t, as)
	-- num clip
	local func = function(ix, no, v, add)
		local px = ":ui/"..v.file
		local x  = v.x + mulpos(add or 0)
		local cl = (v.cx + v.cw * no)..","..v.cy..","..v.cw..","..v.ch
		lyc2{ id=(ix), file=(px), x=(x), y=(v.y), clip=(cl) }
	end

	-- date
	local v  = getBtnInfo('num01')
	local ix = id..".t"
	local n  = t and t.date
	if n then
		local a  = NumToGrph4(n[1])		-- 年
		func(ix..".01", a[1], v, 0)
		func(ix..".02", a[2], v, 15)
		func(ix..".03", a[3], v, 30)
		func(ix..".04", a[4], v, 45)

		local a  = NumToGrph(n[2])		-- 月
		func(ix..".11", a[1], v, 65+1)
		func(ix..".12", a[2], v, 80+1)

		local a  = NumToGrph(n[3])		-- 日
		func(ix..".21", a[1], v, 100+1)
		func(ix..".22", a[2], v, 115+1)

		local a  = NumToGrph(n[4])		-- 時
		func(ix..".31", a[1], v, 135+1)
		func(ix..".32", a[2], v, 150+1)

		local a  = NumToGrph(n[5])		-- 分
		func(ix..".41", a[1], v, 175+1)
		func(ix..".42", a[2], v, 190+1)
	else
		lydel2(ix)
	end

	-- autosave判定
	local hd = game.asavehead
	if as then no = no - hd end

	-- num
	local v  = getBtnInfo('num02')
	local a  = NumToGrph3(no)
	local ax = as and 10 or a[1]
	func(id..".n01", ax  , v, 0)
	func(id..".n02", a[2], v, 15)
	func(id..".n03", a[3], v, 30)
end
----------------------------------------
