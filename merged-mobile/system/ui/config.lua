...
function config_page(page)
	...
	if p == 1 and game.trueos == "windows" then
		conf.window = get_artemis("fullscreen")
		flg.config.vsync = true
	end

	-- aspeed / mspeed
	if p == 1 and game.os == "windows" then
		conf.r_aspeed = 100 - conf.aspeed
		conf.r_mspeed = 100 - conf.mspeed
	elseif p == 2 then
		conf.r_aspeed = 100 - conf.aspeed
		conf.r_mspeed = 100 - conf.mspeed
	end
	...
	----------------------------------------
	-- NIS slider
	if p == init.conf_page_nisslider and checkBtnExist("nis") then
		if shader_confnis() then
			setBtnStat('nis', 'c')
			tag{"lyprop", id=(getBtnID("fl_nis")), visible="0"}
		elseif conf.fl_nis == 0 then
			setBtnStat('nis', 'c')
--			tag{"lyprop", id=(getBtnID("nis")), visible="0"}
		end
	end
	...
	----------------------------------------
	-- 多言語切り替えカーソル制御
	local cl = init.conf_page_lang or 0
	if game.cs and cl == p and bt and bt:sub(1, 4) == "mark" then
		btn_active2(bt)
	end

	----------------------------------------
	-- steamdeckではウィンドウモード切替を無効化（強制フルスクリーン）
	if steam and steam.is_running_on_deck() then
		if checkBtnExist('window1') then
			setBtnStat('window1', 'd')
--			tag{"lyprop", id=(getBtnID("window1")), visible="0"}
		end
		if checkBtnExist('window2') then
			setBtnStat('window2', 'd')
--			tag{"lyprop", id=(getBtnID("window2")), visible="0"}
		end
	end
...
----------------------------------------
function config_pageresetcheck()
	se_ok()
	dialog('pgreset')
end
----------------------------------------
...
--			sch = function()	se_ok() config_samplevoice((gscr.conf.char or 1), p2) end,
			...
			pgreset = function() config_pageresetcheck() end,
...
----------------------------------------
-- config_selectbox
function config_selectbox(e, p)
	local bt = p.btn
	local v  = getBtnInfo(bt)
	local cf = loadBtnData(v.def)		-- read data
	local t  = explode("|", v.p1)
	...
	local id = 600
	local nm = "sbox"
	flg.dlg = { sbox={ name=(btn.name), btn=(p.btn), max=(#t), v=(v), id={} }}
	btn[nm] = { id=(id), p={}, dis={}, key={} }
	btn.name = nm

	-- close btn
	local no = 1
	for i=1, #t do
		if cf == tn2(t[i]) then no = i break end
	end
	local ix = id..".0"
	lyc2{ id=(ix..".0"), file=(px), x=(x), y=(y) }
	btn_selectdraw(ix, v, 'a', no-1)
	lyevent{ id=(ix..'.0'), ix=(ix), name=(nm), btn=(bt), key="btn00", no=(-1), nx=(no), click="config_selectbox_close", over="config_selectbox_over", out="config_selectbox_out"}
	flg.dlg.sbox.id[-1] = ix

	-- select btn
	for i=1, #t do
		local h  = i * v.ch + y
		local ix = id.."."..i
		lyc2{ id=(ix..".0"), file=(px), x=(x), y=(h) }
		btn_selectdraw(ix, v, 'c', i-1)
		flg.dlg.sbox.id[i] = ix

		local bx = string.format("btn%02d", i)
		lyevent{ id=(ix..'.0'), ix=(ix), name=(nm), btn=(bt), key=(bx), no=(i), click="config_selectbox_close", over="config_selectbox_over", out="config_selectbox_out"}
	end

	-- up/dw keyrepeat
	for i, v in pairs(csv.advkey.def) do
		if v.ui == "UP" or v.ui == "DW" then
			tag{"delonpush", key=(i)}
			tag{"setonpush", key=(i), keyrepeat="1", handler="calllua", ["function"]="setonpush_calllua", adv=(v.adv), ui=(v.ui), btn=(v.btn)}
		end
	end
	flip()
end
----------------------------------------
-- selectbox / close
function config_selectbox_close(n)
	local s = flg.dlg.sbox
	btn[btn.name] = nil
	btn.name = s.name

	if n then se_cancel() end

	flg.dlg = nil
	sliderdrag_stat(1)		-- ドラッグ許可
	setonpush_ui()			-- uiに戻す
	lydel2(600)
	tag{"lyprop", id=(getBtnID(s.btn)), visible="1"}

	local fc = s.v.p4
	if fc and _G[fc] then _G[fc]() end
...
----------------------------------------
-- selectbox / click
function config_selectbox_click()
	local s  = flg.dlg.sbox
	local no = s.no
	if no == -1 then
		se_ok()
		config_selectbox_close()
	elseif no then
		local v  = s.v
		local t  = explode("|", v.p1)
		local bc = btn.name
...
----------------------------------------
-- selectbox / over
function config_selectbox_over(e, p)
	local s  = flg.dlg.sbox
	local no = tn(p.no)
	local nx = tn(p.nx or no)
	local cl = no > 0 and 'd' or 'a'
	btn_selectdraw(p.ix, s.v, cl, nx-1)
	flip()
...
----------------------------------------
-- selectbox / out
function config_selectbox_out(e, p)
	local s  = flg.dlg.sbox
	local no = tn(p.no)
	local nx = tn(p.nx or no)
	local cl = no > 0 and 'c' or nil
	btn_selectdraw(p.ix, s.v, cl, nx-1)
	flip()
...
----------------------------------------
function config_selectbox_up(e, p) config_selectbox_cursor(-1) end	-- selectbox / up
function config_selectbox_dw(e, p) config_selectbox_cursor(1) end	-- selectbox / dw
----------------------------------------
-- selectbox / cursor
function config_selectbox_cursor(add)
	local s  = flg.dlg.sbox
	local mx = s.max
	local z  = s.id
	local p  = s.pp
	...
	-- out
	if p then config_selectbox_out(e, p) end

	-- 初回
	if not no or no == -1 then
		if add < 0 then no = mx else no = 1 end
		config_selectbox_over(e, { ix=(z[no]), no=(no) })
...
function conf_langchange(e, p)
	...
	if bt then
		...
		if z[p3] then
			...
			lydel2(init.mwid..".mw.bt")			-- mw削除 / btn
			lydel2(init.mwid..".tb")			-- mw削除 / tablet
			lydel2(500)							-- ui削除
			flip()								-- flipしておかないと残像が残る
...
----------------------------------------
-- キーで言語切り替え(トグル)
function config_key_langtoggle(e, p)
	-- lang_change()と同じ判定
	...
	local bt = "bt101"..(2 + num)
	config_key_lang_bttoggle(bt)
end
----------------------------------------
-- キーで言語切り替え(数値)
function config_key_langnum(e, p)
	local key_bt = getBtnInfo(p.bt)
	config_key_lang_bttoggle(key_bt and key_bt.p1)
end
----------------------------------------
-- キーで言語切り替え時にconf_toggleと同じ処理をする関数
function config_key_lang_bttoggle(bt)
	if bt then
		local v = getBtnInfo(bt)
		local a = explode("|", v.p2)
		for i, nm in pairs(a) do
