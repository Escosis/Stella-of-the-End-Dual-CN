...
function getTweenTime(time, def)
	...
	if flg.fastauto then
		tm = getFastautoTime(tm)
	elseif tm and getSkip() then
		tm = 0
	end
	return tm
end
----------------------------------------
-- fastauto time
function getFastautoTime(time)
	local fl = conf.fl_fastauto or 1
	local tm = conf.fastauto or 50
	if time > 0 and fl == 1 then
		time = math.floor(time * tm / 1000)
	end
	return time
end
----------------------------------------
...
function trans_main(p)
	...
	if flg.fastauto and time > 0 then time = getFastautoTime(time) end

	-- ruleが指定されているかどうか
	local r = getRule(p, true)
	local s = p.rule or ""
--	if time == 0 or not r or s:find("sig_") then
	if time == 0 or not r or getSkip() or shader_rule(p, time) then
