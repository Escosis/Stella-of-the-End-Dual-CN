...
function sesys_play(mode, file, p)
	local file = sesys_langpath(mode, file)		-- 多言語path
...
--	if gain == 0 then return end				-- 抜けてしまうとconfig終了時にloopseが復帰できなくなる
	...
	----------------------------------------
	-- 既に同ファイル再生中
	local gain2 = p.gain2 or gain
	if file == s.file then

		-- gain
		if p.vol then
			tag{"sefade", id=(id), gain=(gain2), time=(time)}
...
--		message("再生", mode, file, id, skbl, gain)
		...
		-- fade処理(noise対策)
		local tm = time or 0
		local mx = init.game_se_fadein or 0
		if tm < mx then tm = mx end

		-- PS4は無音ファイルを一瞬置くことで安定する
		if game.os == "ps4" then tag{"seplay", id=(id), file=":sysse/ps4.ogg", gain=(gain), time="0"} end
		tag{"seplay", id=(id), file=(file), gain=(gain2), time=(tm), loop=(loop), skippable=(skbl), buffer=(sesys_getbuffer(p))}
...
--				sesys_play("lvo", path, p)
				...
				-- save
				if not scr.svlvo then scr.svlvo = {} end
				scr.svlvo[ch] = { path=(path), p=(p) }
			end
		else
...
----------------------------------------
-- 積んだbgvを回す
function sesys_lvoloop()
	local z = scr.svlvo
	if z then
		local s = scr.sesys
		for ch, v in pairs(z) do
			local p = v.p
			if s then
				local ch = p.ch
				for i, t in pairs(s.buff) do
					if ch == t.ch and t.mode == "voice" then p.gain2 = 0 end
				end
			end
			sesys_play("lvo", v.path, p)
		end
		scr.svlvo = nil
	end
end
...
function sesys_vostack(p)
	...
	for i, v in pairs(p) do
		...
		-- lvo処理
		local ch = v.ch
		if z[ch] then scr.svlvo[ch].p.gain2 = 0 end
	end
end
----------------------------------------
-- 積んだ音声を回す
function sesys_voloop()
	...
	elseif scr.vo then
		...
		if conf.voiceskip == 0 or conf.autostop == 0 then
			...
			local t  = ast[bl].text or {}
			if not t.clickpass then sesys_stop("voice") end
...
function sesys_getbuffer(p)
	...
	-- 設定不能なOSは通知を出す
	elseif soundbuffer[oz] then
		if z.bgmbuff or z.sebuff then
			message("警告", oz, "ではsoundbufferを指定できません")
		end

	-- SE設定
	elseif type(p) == "table" then
		r = p.buffer or z.sebuff
...
----------------------------------------
-- 多言語pathの追加
function sesys_langpath(mode, file)
	local fl = file
	local ln = get_language(mode)
	local z  = init.langvo or {}
	if ln and z[ln] and z[ln][mode] then
		local ad = z[ln][mode] or ""
		local a  = explode("/", fl)
		local mx = #a
		a[#a] = ad..a[#a]
		fl = implode("/", a)
