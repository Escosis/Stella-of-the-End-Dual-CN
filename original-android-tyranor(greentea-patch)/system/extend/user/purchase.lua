...
----------------------------------------
-- purchaseタグ共通化
function purchase_gettag(id, no)
...
function purchase_inquiry(productid)
	flg.purchase = nil
	local p = purchase_gettag(productid, "0")	-- 確認
	eqtag(p)
...
function purchase_buy(productid)
	flg.purchase = nil
	local p = purchase_gettag(productid, "1")	-- 購入
	eqtag(p)
	eqtag{"calllua", ["function"]="purchase_inquirynext"}
end
----------------------------------------
...
function purchase_inquirynext(e, p)
	...
	--else
		--flg.purchaseerror = true	-- error flag
		--tag_dialog({ title="error", message="purchase99", num=(code) })
	--end
...
function purchase_restorecheck()
	...
	--if max < 0 then
	--	flg.purchaseerror = true	-- error flag
	--	tag_dialog({ title="notice", message="purchase91" })	-- 購入済みのアイテム情報が取得できませんでした。
	--elseif max == 0 then
	--	flg.purchaseerror = true	-- error flag
	--	tag_dialog({ title="notice", message="purchase92" })	-- 購入済みのアイテムはありませんでした。
